...
diff_LG_trust <-
  all_rounds |>
  filter(country_name %in% c("Botswana", "Ghana", "Lesotho", "Malawi", "Mali", "Namibia", "Nigeria", "South Africa", "Tanzania", "Uganda", "Zambia", "Zimbabwe", "Kenya", "Mozambique", "Senegal")) |>
  filter(LG_trust %in% c("2", "3")) |>
  group_by(round, country_name) |>
  summarize(n = n()) |>
  mutate(LG_trust_high = n / sum(n)) |>
  select(-n) |>
  pivot_wider(names_from = round, values_from = LG_trust_high) |>
  mutate(diff_LG_trust = `r7` -`r2`)
...
diff_TL_trust <-
  ...
  group_by(round, country_name) |>
  summarize(n = n()) |>
  mutate(TL_trust_high = n / sum(n)) |>
  select(-n) |>
  pivot_wider(names_from = round, values_from = TL_trust_high) |>
  mutate(diff_TL_trust = `r7` -`r2`)
```


```{r}
diff_all_rounds <-
  full_join(diff_LG_trust, diff_TL_trust, by = "country_name") 

diff_all_rounds <- na.omit(diff_all_rounds)
```




## how to omit a response "9" from the data.
filter(the variable name!=9)
filter(LG_trust!=9)
